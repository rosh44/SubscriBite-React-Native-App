...
import { View, Text, FlatList, StyleSheet, SafeAreaView, Modal, Pressable } from 'react-native';
import { Image } from 'react-native-elements';
import Icon from 'react-native-vector-icons/Feather';
import IncDecButton from '../ui/IncDecButton';
import LeftRight from '../ui/LeftRight';

const movement = ["06 AM - 09 AM", "09 AM - 12 PM", "12 PM - 03 PM", "03 PM - 06 PM"]
const movement_mapper = { "06 AM - 09 AM": 1, "09 AM - 12 PM": 2, "12 PM - 03 PM": 3, "03 PM - 06 PM": 4 }
...
const SubscriptionList = (props) => {
  ...
  useEffect(() => {
    ...
    if (dateData) {
      org_data = dateData.reduce((acc, item) => {
        acc[item.id] = { "time_slot_description": item.time_slot_description, "quantity": item.quantity }
        return acc;
      }, {}
      )
      ...
      for (const key in changed_data) {
        changed_data[key]['change'] = (function (key) {
          if (org_data[key]['quantity'] != changed_data[key]['quantity'] || org_data[key]['time_slot_description'] != changed_data[key]['time_slot_description']) {
            return true
          }
          return false
        })(key)
  ...
  function IncDecUpdate(id, quantity) {
    ...
    if (changed_data[id]) {
      ...
      if (
        ...
        org_data[id]['time_slot_description'].trim() === changed_data[id]['time_slot_description'].trim()
      ) {
    ...
    if (overallState) {
      ...
      for (let i in changed_data) {
        if (changed_data[i]['change']) {
          create_data_to_push['data'].push({ 'id': i, 'quantity': changed_data[i]['quantity'], 'slot': movement_mapper[changed_data[i]['time_slot_description']] })
        }
  ...
  const renderItem = ({ item }) => {
    return (
      <View style={[{flex:1, flexDirection:'row', },styles.itemsMainContainer]}>
                  ...
                  <View styles={{flex:1}}>
                    <View style={styles.itemContainer}>
                      ...
                      <View style={styles.description}>
                        ...
                        <IncDecButton quantity={item.quantity} uniqueId={item.id} parentUpdateFunc={IncDecUpdate} style={{ padding: 0 }} />
                        <LeftRight slotValue={item.time_slot_description} movement={movement} uniqueId={item.id} parentUpdateFunc={LeftRightUpdate} />
                      </View>
                    </View>
                  </View>
                </View>
        ...
        {/* </View> */}
      <View style={{flex:1, flexDirection:'column', backgroundColor:'white', padding:4}}>
        <View style={{ flex: 6,backgroundColor:'white'}}>
              <Image style={{width: '100%',height: '100%', alignItems:'center', }} source={{uri: item.img_url }} resizeMode="cover"/>
        </View>
        <View style={{ flex: 1,backgroundColor:'white', alignItems:'center', justifyContent:'flex-end', alignContent:'center', fontWeight:'bold'}}>
              <Text style={{fontWeight:'bold'}}>${(((changed_data[item.id])?changed_data[item.id]['quantity']:item.quantity||quantChange) * item.price).toFixed(2)}</Text>
        </View>
      </View>
  ...
  return dateData != null ? (
      ...
      <View style={{ flex: 8, flexDirection: 'row' }}>
        ...
        (
          ...
          <View style={{flex:1, flexDirection:'row', justifyContent:'center', alignItems: 'center',alignContent:'center'}}>
            <View style={{flex:1}}>
              <Text style={{fontWeight:"bold", alignContent:'center',justifyContent:'center',alignSelf:'flex-end', fontSize:18}}>Save</Text>
            </View>
            <View style={{ flex: 1, flexDirection: 'row', alignItems: 'flex-start' }}>
              <Pressable onPress={() => { saveTheStateToDB() }}>
    ...
    (
      <View style={{ alignItems: 'center' }}>
        <Image source={require('../../assets/noOrders.png')} style={styles.imageView}/>
